...
interval (length-$3$ blocks over the per-date series) to every comparison and apply
...
bed this flips negative semibeta, a co-crash shape metric, from a head-to-head $-0.067$
against volatility (CI $[-0.131,-0.003]$, significantly worse) to $+0.055$
(CI $[-0.082,0.178]$, a tie), closes downside beta from $-0.23$ to $-0.01$, and lifts
VaR's partial from $+0.135$ to $+0.205$ (Table~\ref{tab:ablation}). Survivorship bias erases shape's penalty and
inflates the magnitude edge, precisely in the crash regime where hiding the coins that
died most reshapes the cross-section. It does not manufacture a winner. No shape metric
beats volatility even survivors-only, and the rest stay far below it.

\begin{table}[t]
\centering
\small
\caption{Survivorship ablation on the crypto-2018 crash bed. Removing the coins that died
(survivors-only) turns a co-crash shape metric's loss into a tie and inflates the
magnitude edge, yet no shape metric beats volatility either way.}
\label{tab:ablation}
\begin{tabular}{lcc}
\toprule
crypto-2018, vs.\ volatility & full universe & survivors-only \\
\midrule
neg.\ semibeta (head-to-head) & $-0.067$ (worse) & $+0.055$ (tie) \\
downside beta (head-to-head) & $-0.23$ & $-0.01$ \\
