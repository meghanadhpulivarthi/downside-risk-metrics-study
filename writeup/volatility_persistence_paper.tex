...
subtracts. Yet the shape metrics are not broken. On genuinely
...
block-bootstrap confidence intervals (CIs), and Benjamini--Hochberg (BH) correction.
...
integrity of the evaluation behind it. (1) A clean empirical law we call magnitude, not shape. The only signal surviving correction beyond volatility is the \emph{size}
of tail losses (VaR/ES/downside deviation), not their asymmetry (\S\ref{sec:magnitude}).
(2) Volatility is hard to beat because it is persistent, though a predictive
test shows forward volatility is not a sufficient statistic (\S\ref{sec:why}). (3) The
residual edge does not pay its way, except in fat-tailed crypto (\S\ref{sec:why}).
(4) An autonomous large-language-model (LLM) discovery loop does not beat volatility
...
magnitude edge (App.~\ref{app:ablation}), so the small equity increments are plausibly
upper bounds and shape's equity penalty a lower bound.
...
and Benjamini--Hochberg (BH) correction at $q=0.05$ applied within each test battery (the
head-to-head and the partial family, ${\sim}45$ cells each; BH runs within battery because
the two answer different questions). The block bootstrap uses length-$3$ blocks over the
per-date series, near the $n^{1/3}$ rule of thumb; the VaR increment's sign and
significance are unchanged under block lengths $1$--$4$ and a stationary bootstrap
(App.~\ref{app:partials}). On the thin crypto beds ($7$--$9$ dates) this grid is
coarse, so we back every crypto star with its interval and minimum detectable effect (MDE)
...
tail-exceedance frequency, the shape metrics do beat volatility on nearly every
...
leading but incomplete explanation, not a mechanism that reduces drawdown to
one number.
...
z-score (each metric standardized across the assets on a given date). We fixed VaR as the
composite's second term before the locked panel was opened. On a locked panel it shows no
significant out-of-sample gain over volatility (Newey--West $t=0.67$, lag $3$), beating it
only on fat-tailed crypto-bull (Table~\ref{tab:econ}). That crypto-bull edge is $+0.05$,
below the bed's $80\%$-power MDE of $0.09$, so like VaR's crypto claims it is suggestive
rather than powered. A cost-aware tercile backtest agrees. The
...
\emph{locked-test} score plateaus at the volatility baseline (Figure~\ref{fig:bon}). The
agent is Claude Opus 4.8, given a frozen scorer, a train/validation split, and one file
defining the metric. Each iteration it reads the log of past attempts, rewrites the metric
with one new idea (any Python over the per-row features volatility, ES, VaR, LPM$_2$,
downside deviation, trailing drawdown, skew, kurtosis, and market cap, fitting parameters
on train only), scores it on validation, and keeps the edit only if validation improved;
the run we report ran $51$ iterations ($14$ kept). Across $12$ holdout
panels $\times$ $3$ search seeds (App.~\ref{app:snooping}), the validation-selected
metric's out-of-sample edge over volatility is small but statistically positive, averaging
$+0.020$ out-of-time (clustered $t=4.3$) and $+0.035$ under random splits ($t=5.3$). The
out-of-time edge is consistent with search rediscovering the same tail-magnitude increment
of \S\ref{sec:magnitude}, not new signal, while the larger random-split edge is a signature
of temporal leakage; a non-LLM sweep reproduces both, so the danger is the cheap
search, not the language model. The statistics are classical data
snooping~\cite{white2000,sullivan1999,bailey2017}; what is new is that an autonomous agent
removes the last brake on trial count (analyst effort), so a locked, survivorship-free
...
splits, the signature of temporal leakage (App.~\ref{app:snooping}).}
\label{fig:bon}
\end{figure*}
...
it also carries downside magnitude, as in negative semibeta; pure shape adds nothing. The fair-field disciplines change what one would report, not the verdict.
...
Code, data pointers, a pre-registration record (the frozen tests, the generalizability
bar, and the composite), and result logs to reproduce every table and figure are
...
effect ($0.13$) is below its MDE ($0.18$) and is reported as suggestive. VaR's increment
is also stable across block lengths $1$--$4$ and a stationary bootstrap, with the $95\%$ CI
strictly positive on all five beds. A placebo
...
the $95\%$ CI on every shape increment excludes effects larger than $\approx0.04$ (the
$80\%$-power minimum detectable increment is $0.034$--$0.042$; Table~\ref{tab:partials}), so
on equity ``shape adds nothing'' is an equivalence result, not a failure to detect.
...
between beds, the honest test clusters by bed, where the slope is positive
...
The same pattern holds within crypto. Splitting the crypto dates at their median
...
and small ($+0.056$ vs.\ $+0.050$; $120$ dates). Within crypto as across beds, then, the
increment grows precisely when the tail turns active.
...
cross-sectionally per date, with no fitting. VaR was fixed as the composite's second term
before the locked panel was opened. It was the only metric whose increment survived
correction on all five exploratory beds (\S\ref{sec:magnitude}); the
weights are untuned, so the locked panel is fully out-of-sample
(Table~\ref{tab:econ}, \S\ref{sec:why}), and its gain is not significant. The economic
backtest (Figure~\ref{fig:economic}) sorts the universe into risk
terciles and measures realized-drawdown discrimination (forward DD of the high-
minus low-risk tercile) and the forward-return spread, net of a $20$~bps cost charged on
...
\emph{Without a locked holdout.} The discovery experiment
(App.~\ref{app:snooping}, Table~\ref{tab:bon} and Figure~\ref{fig:bon})
...
the best validation score with search intensity while the locked test
...
\emph{The gaps are small but positive.} The test gap averages $+0.020$ out-of-time (range
$-0.028$ to $+0.046$; clustered $t=4.3$ across the six panels) and $+0.035$ under random
repartitions ($+0.014$ to $+0.072$; $t=5.3$). Both are statistically positive. The
out-of-time gap is consistent with search rediscovering the tail-magnitude increment of
\S\ref{sec:magnitude}, not new signal, while the random-split gap is the larger of the two,
a signature of temporal leakage, because order-ignoring repartitions interleave validation
and test dates and let volatility's persistence leak across the split. Validation--test
rank correlation is $0.94$--$0.99$ throughout.
\emph{The win-rate is noisier.} It averages $0.79$ (out-of-time, range $0.55$--$1.00$) and
...
on calm windows. Because these nominal ``wins'' come with a tiny gap that, out-of-time, is
consistent with the documented magnitude increment, the \emph{sign} of the win-rate is far
less informative than its \emph{magnitude}. The validation-selected metric essentially
matches volatility, edging it only by that sliver.
