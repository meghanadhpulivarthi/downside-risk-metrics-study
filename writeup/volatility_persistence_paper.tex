...
question is which assets to derisk on a given date. The remaining machinery
...
The verdict is blunt. After correction, no metric beats volatility in a way that
...
clears it. That is narrower and more defensible than the flat claim that nothing beats
volatility. Small, regime-specific edges exist for volatility's own variants, but none
survives a change of asset class. The verdict, the magnitude-vs-shape partials, and the
composite were pre-registered, while heterogeneity and stress-conditional cuts are
exploratory.
...
ES~\cite{rockafellar2000} & 0.62 & 0.35 & 0.33$\dagger$ & 0.58$\dagger$ & 0.52$\dagger$ \\
...
Two clarifications. Start with an apparent paradox. VaR is \emph{worse} than volatility
...
ES~\cite{rockafellar2000} & $+0.14\ast$ & $+0.08\ast$ & $+0.08$ & $+0.03$ & $+0.00$ \\
...
volatility on a locked panel (Newey--West $t=0.67$, lag $3$), beating it only on fat-tailed
crypto-bull (Table~\ref{tab:econ}). A cost-aware tercile backtest agrees. The
...
\emph{Limitations.} The performance measure is rank correlation, not a full tradable P\&L
...
drawdown forecasting, not the originals' pricing claims. Finally, a survivorship-free
\emph{equity} bed would require CRSP-grade delisted prices, since the free sources we
tried (Kaggle, Stooq) do not carry the window's S\&P~500 casualties.
...
fat-tailed crypto beds and consistent with the conditional evidence in
Appendix~\ref{app:conditional}.
...
The three fair-field disciplines are not decorative. We ask which shape metric one
would have credited without each.
